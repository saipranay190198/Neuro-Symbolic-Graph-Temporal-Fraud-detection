...
# Assumes df_sample (pandas DataFrame) is present in the environment
# and node_amounts (pandas Series) is available. If not, we recompute.
...
# -------------------------
# 0. Quick checks / prepare
# -------------------------
assert 'sender' in df_sample.columns and 'receiver' in df_sample.columns,     "df_sample must contain 'sender' and 'receiver' columns."

# Recompute node_amounts if not present
if 'node_amounts' not in globals():
    node_amounts = (
        df_sample.groupby('sender')['amount'].sum().add(
        df_sample.groupby('receiver')['amount'].sum(), fill_value=0)
    )

# Build node list (unique nodes in sample)
nodes = pd.Index(np.unique(pd.concat([df_sample['sender'], df_sample['receiver']])))
node_to_idx = {node: i for i, node in enumerate(nodes)}
n_nodes = len(nodes)
print(f"Number of nodes in sampled graph: {n_nodes}")

# -------------------------
# 1. Build NetworkX directed graph for feature engineering
# -------------------------
G = nx.DiGraph()
G.add_nodes_from(nodes)
for _, row in df_sample.iterrows():
    s = row['sender']; r = row['receiver']; amt = float(row['amount'])
    # add edge; if multiple edges exist they're aggregated w/ weight
    if G.has_edge(s, r):
        G[s][r]['weight'] += amt
        G[s][r]['count'] += 1
    else:
...
# -------------------------
# Basic numeric features (can be extended)
total_amount = np.array([float(node_amounts.get(node, 0.0)) for node in nodes])
in_deg = np.array([G.in_degree(node) for node in nodes], dtype=float)
out_deg = np.array([G.out_degree(node) for node in nodes], dtype=float)
degree = in_deg + out_deg
pagerank_dict = pagerank_safe(G)
pagerank = np.array([pagerank_dict[node] for node in nodes]) if n_nodes > 1 else np.zeros(n_nodes)


# Normalize numeric features
scaler = StandardScaler()
node_features_numeric = scaler.fit_transform(
    np.vstack([total_amount, in_deg, out_deg, degree, pagerank]).T
)

# Optionally add a one-hot indicator for "involved in fraud" at node-level as label
node_is_fraud = np.zeros(n_nodes, dtype=int)
for i, node in enumerate(nodes):
    # if node occurs in any fraudulent transaction in df_sample
    is_f = df_sample[
        ((df_sample['sender'] == node) | (df_sample['receiver'] == node)) &
        (df_sample['is_fraud'] == 1)
    ].any(axis=None)
    node_is_fraud[i] = 1 if is_f else 0

print("Feature summary: total_amount (scaled), in_deg, out_deg, degree, pagerank")
...
# -------------------------
# 3. Build PyG graph (edge_index)
# -------------------------
# Convert edges to index form (keep direction)
edge_index_list = []
edge_attr = []
for u, v, data in G.edges(data=True):
    ui = node_to_idx[u]; vi = node_to_idx[v]
    edge_index_list.append([ui, vi])
    edge_attr.append([data.get('weight', 0.0), data.get('count', 1)])
...
x = torch.tensor(node_features_numeric, dtype=torch.float)  # shape [N, F]
y = torch.tensor(node_is_fraud, dtype=torch.long)           # shape [N]
...
class GraphSAGE(torch.nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        return x  # final node embeddings (logits if used for classification)
...
# -------------------------
# 5. Train GraphSAGE for node-level classification
# -------------------------
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
model = GraphSAGE(in_channels=data.num_node_features, hidden_channels=64, out_channels=32).to(device)
data = data.to(device)

optimizer = torch.optim.Adam(model.parameters(), lr=0.01, weight_decay=5e-4)
criterion = torch.nn.BCEWithLogitsLoss()  # using logits for binary node labels

# create train/test split of nodes (stratified)
train_idx, test_idx = train_test_split(np.arange(n_nodes), test_size=0.3, stratify=node_is_fraud, random_state=42)
...
for epoch in range(1, epochs + 1):
    optimizer.zero_grad()
    out = model(data.x, data.edge_index).squeeze()       # shape [N, out_dim]
    # For classification, reduce out to a single logit per node using a small linear head
    # Simple approach: take dot with a learnable vector to map embedding -> logit
    # We'll create a linear head on the fly:
    # (Better: define linear head in model; for brevity we use a small projection here.)
    # But to keep training end-to-end, add a linear layer:
    # Re-define with small classifier:
    break

# Rebuild model with classification head properly
class GraphSAGEWithHead(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, hidden_emb, dropout=0.3):
        super().__init__()
        self.sage1 = SAGEConv(in_channels, hidden_channels)
        self.sage2 = SAGEConv(hidden_channels, hidden_emb)
        self.cls = torch.nn.Linear(hidden_emb, 1)
        self.dropout = dropout
    ...
    def forward(self, x, edge_index):
        ...
        logits = self.cls(h).squeeze(-1)  # [N]
        return logits, h  # logits for classification, h as embeddings

model = GraphSAGEWithHead(in_channels=data.num_node_features, hidden_channels=64, hidden_emb=32).to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=0.01, weight_decay=5e-4)
...
for epoch in range(1, epochs + 1):
    ...
    loss = criterion(logits[train_mask], y_float[train_mask])
    loss.backward()
    optimizer.step()
    ...
    if epoch % 20 == 0 or epoch == 1:
        ...
        with torch.no_grad():
            ...
            val_pred = (torch.sigmoid(val_logits[test_mask]) > 0.5).cpu().numpy().astype(int)
            val_true = y[test_mask.cpu()].cpu().numpy().astype(int)
...
# -------------------------
# 6. Extract embeddings and produce node-level dataset for SRI
# -------------------------
model.eval()
with torch.no_grad():
    logits_all, embeddings_all = model(data.x, data.edge_index)
    probs = torch.sigmoid(logits_all).cpu().numpy()
    embeddings_np = embeddings_all.cpu().numpy()  # shape [N, emb_dim]

# Build a pandas DataFrame containing engineered features + embeddings + label
feat_df = pd.DataFrame({
    ...
    'degree': degree,
    'pagerank': pagerank,
    'fraud_label': node_is_fraud,
    'graphsage_prob': probs
})

# attach embedding columns
emb_cols = [f'emb_{i}' for i in range(embeddings_np.shape[1])]
emb_df = pd.DataFrame(embeddings_np, columns=emb_cols)
feat_df = pd.concat([feat_df.reset_index(drop=True), emb_df.reset_index(drop=True)], axis=1)
...
# -------------------------
# 7. Symbolic Rule Inducer: Decision Tree on combined features
# -------------------------
# Choose a concise set of interpretable features + top few embedding dims
selected_features = ['total_amount', 'in_deg', 'out_deg', 'degree', 'graphsage_prob'] + emb_cols[:6]  # keep 6 embedding dims
X = feat_df[selected_features].fillna(0).values
y_sri = feat_df['fraud_label'].values

# split
X_train, X_test, y_train, y_test = train_test_split(X, y_sri, test_size=0.3, stratify=y_sri, random_state=42)

sri_clf = DecisionTreeClassifier(max_depth=4, min_samples_leaf=5, random_state=42)
sri_clf.fit(X_train, y_train)

y_pred = sri_clf.predict(X_test)
print("\nDecision Tree (SRI) classification report:")
print(classification_report(y_test, y_pred))
print("Accuracy:", accuracy_score(y_test, y_pred))

# Export rules
feature_names = selected_features
rules_text = export_text(sri_clf, feature_names=feature_names)
print("\n--- Symbolic Rules (Decision Tree) ---\n")
print(rules_text)

# Save artifacts
os.makedirs('model_artifacts', exist_ok=True)
np.save('model_artifacts/node_embeddings.npy', embeddings_np)
feat_df.to_csv('model_artifacts/node_features_with_embeddings.csv', index=False)
with open('model_artifacts/sri_rules.txt', 'w') as f:
    f.write(rules_text)

print("\nArtifacts saved to ./model_artifacts/ (embeddings CSV, SRI rules)")
...
# X = feature matrix (embeddings, degrees, totals, etc.)
...
# Example: embeddings + degrees + other graph features
# NumPy array X.shape = (n_samples, 11)
feature_names = [
    'emb_0', 'emb_1', 'emb_2', 'emb_3', 'emb_4',  # 5 embedding dims
    'in_deg', 'out_deg', 'degree',                # 3 degree features
    'total_amount',                               # transaction amount
    'pagerank',                                   # graph feature
    'clustering_coef'                             # graph feature
]
...
print("--- Symbolic Rules (Decision Tree) ---\n")
print(tree_rules)
...
def predict_account(in_deg, emb_0, degree):
    if in_deg <= 1.31 and emb_0 <= 3511.47 and degree <= -1.41:
        return 1  # fraud
    else:
        return 0  # non-fraud
...
predict_account(1.01, 3211.47, 1.43)
